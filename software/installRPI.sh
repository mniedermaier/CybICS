...
ssh "$DEVICE_USER"@"$DEVICE_IP" /bin/bash <<EOF
    set -e
    # /boot/firmware/config.txt, not /boot/config.txt. Since Bookworm the
    # latter is a 91-byte stub reading "DO NOT EDIT THIS FILE - The file you
    # are looking for has moved to /boot/firmware/config.txt", so appending
    # there put gpu_mem in a file the firmware never reads and the setting
    # silently did nothing. The cmdline edit above already uses the right path.
    grep -qF -- 'gpu_mem=16' '/boot/firmware/config.txt' || echo 'gpu_mem=16' | sudo tee -a '/boot/firmware/config.txt' > /dev/null
EOF
