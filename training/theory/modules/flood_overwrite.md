...
The PLC program only ever reads 1126; what keeps it truthful is `hwio`, whose loop does a coil read and five register writes and then sleeps 20 ms &mdash; about 21.5 ms a pass, measured. A single forged write survives roughly ten milliseconds and is gone.
...
<figure>
<style>
.fo-r {--d: 12s; --on:#ff6b00;}
html.light-mode .fo-r {--on:#b34700;}
...
<svg class="fo-r" viewBox="0 0 460 196" role="img"
     aria-label="A hundred-millisecond timeline. The attacker writes register 1126 about every 1.2 milliseconds; hwio writes the true value back about every 21 milliseconds. The register therefore holds the forged value for all but about one millisecond in twenty.">
  ...
  <text x="8" y="110" font-size="13" opacity="0.7">~21 ms</text>
...
<figcaption>The orange band is the forged value; the dark notches are the moments <code>hwio</code> gets its measurement in before the next attacker write buries it. <code>hwio</code> owns the word about 5.6% of the time &mdash; and that number comes back below, because it is exactly the attacker's residual loss.</figcaption>
</figure>
...
There are drop-outs, and their cause is the figure above. The PLC scans every 50 ms; `hwio` owns register 1126 for about 5.6% of any interval. So roughly one scan in eighteen samples the *true* pressure, both compressor conditions fail at once, the `ELSE` fires, and the compressor stops for exactly one scan. That predicts a duty cycle of 94.4%; measured over a 400-second flood it is **94.6%**. The attacker's 5.6% residual loss on the wire is the controller's 5.4% residual sanity.

A compressor running nineteen scans in twenty, feeding a tank with no outlet. The attacker never touched the compressor coil or the valve coil.
...
<svg class="fo-d" viewBox="0 0 460 210" role="img"
     aria-label="Two traces over about five minutes. The value OpenPLC reads from register 1126 stays flat at 10. The pressure actually in the tank climbs from 75, reaching the relief valve at about three minutes and the 255 ceiling at about four and a half. The compressor runs nineteen scans in twenty with the system valve shut.">
  <line x1="60" y1="30" x2="60" y2="155" stroke="currentColor" stroke-opacity="0.4"/>
  <line x1="60" y1="155" x2="340" y2="155" stroke="currentColor" stroke-opacity="0.4"/>
  <g font-size="13" opacity="0.7">
    <text x="54" y="34" text-anchor="end">255</text>
    <text x="54" y="51" text-anchor="end">220</text>
    <text x="54" y="154" text-anchor="end">10</text>
    <text x="60" y="174" text-anchor="middle">0</text>
    <text x="200" y="174" text-anchor="middle">2.5 min</text>
    <text x="340" y="174" text-anchor="middle">5 min</text>
  </g>
  ...
  <rect x="60" y="186" width="280" height="14" rx="3" fill="var(--on)"/>
  <text x="200" y="197" text-anchor="middle" font-size="13" style="fill:#1a1a1a" font-weight="bold">compressor on 19 scans in 20, valve shut</text>
  <line class="head" x1="60" y1="26" x2="60" y2="161" stroke="currentColor" stroke-width="2"/>
</svg>
<figcaption>The physics advances one tick per <em>fifty-one</em> passes of <code>hwio</code>'s outer loop &mdash; the `if delay > 50` gate &mdash; so a tick is about 1.1 seconds, not 21 milliseconds. That is why a millisecond-scale race has a minute-scale consequence: roughly three minutes to the relief valve and four and a half to the ceiling, measured. Above the dashed line the valve halves the rate of rise and no more, which the measurement confirms exactly: 0.825 units per second below it, 0.391 above.</figcaption>
</figure>
...
.fo-c .fill {transform-box: fill-box; transform-origin: left;
             animation: c-fill var(--d) linear 30 forwards;}
.fo-c .tick {transform-box: fill-box; transform-origin: bottom;
             animation-duration:var(--d); animation-timing-function:steps(1,end);
             animation-iteration-count:30; animation-fill-mode:forwards;}
.fo-c .t1{animation-name:c-t1} .fo-c .t2{animation-name:c-t2} .fo-c .t3{animation-name:c-t3}
.fo-c .alarm{animation: c-alarm var(--d) steps(1,end) 30 forwards;}
.fo-c .slow {animation: c-slow var(--d) steps(1,end) 30 forwards;}
...
Run `flooding_hpt.py` against the PLC, watch FUXA's blow-out flag go high while its HPT trend still reads 10, and click Verify. The flag comes from the IDS having seen `modbus_flood` on the wire, not from the plant reaching any particular state. Note that the plant does not recover on its own afterwards: with the valve shut above 100 there is no drain path, so the tank sits where the attack left it until something restarts `hwio`.
