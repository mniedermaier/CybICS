...
Uses Playwright's built-in video recorder with CSS tweaks for smoother animations.
"""
...
# CSS injected to make animations smoother at lower capture frame rates.
# Slows down fast animations and uses linear easing (less jarring at low fps).
SMOOTH_CSS = """
/* Slow down the floating icon animation so it looks smooth at low fps */
.service-box-icon {
...
/* Slow down floating orbs */
body::before {
    animation-duration: 40s !important;
}
/* Disable the typewriter effect — it flickers at low fps */
.home-header h2 {
...
def inject_smooth_css(page):
    """Inject CSS to slow down animations for smoother video capture."""
    page.add_style_tag(content=SMOOTH_CSS)
